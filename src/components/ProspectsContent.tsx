...
export function ProspectsContent() {
  ...
  // MILESTONE 5C — filtering by a single product could otherwise mean
  // scrolling through dozens of prospects at once (a whole book's worth
  // was 39). 10 per page keeps any one filtered view neat regardless of
  // how many prospects accumulate for it.
  const PROSPECTS_PER_PAGE = 10;
  const [currentPage, setCurrentPage] = useState(1);

  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, productFilter, filterIndex]);

  const totalPages = filteredProspects
    ? Math.max(1, Math.ceil(filteredProspects.length / PROSPECTS_PER_PAGE))
    : 1;

  const paginatedProspects = useMemo(() => {
    if (!filteredProspects) return filteredProspects;
    const start = (currentPage - 1) * PROSPECTS_PER_PAGE;
    return filteredProspects.slice(start, start + PROSPECTS_PER_PAGE);
    ...
    <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          className="w-full rounded-full sm:ml-auto sm:w-64 border border-ink-600 bg-ink-800 px-3 py-1 text-xs text-white placeholder:text-white/30 focus:border-emerald-500/50 focus:outline-none"
        />
      </div>
      ...
      {filteredProspects && filteredProspects.length > PROSPECTS_PER_PAGE && (
        <div className="mt-4 flex items-center justify-between text-xs text-white/50">
          <span>
            Page {currentPage} of {totalPages} — {filteredProspects.length} prospects
          </span>
          <div className="flex gap-2">
            <button
              disabled={currentPage === 1}
              onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
              className="rounded-md border border-ink-600 px-3 py-1 hover:text-white disabled:opacity-30"
            >
              Previous
            </button>
            <button
